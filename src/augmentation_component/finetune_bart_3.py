...
class CustomBart(BartForConditionalGeneration):
    ...
    def _get_combined_encoder_outputs(
        ...
        return combined_encoder_outputs, combined_attention_mask

    def forward(
        self,
        query_input_ids: torch.LongTensor = None,
        query_attention_mask: torch.Tensor = None,
        retr_input_ids: torch.LongTensor = None,
        retr_attention_mask: torch.Tensor = None,
        ...
        **kwargs,
        ...
        if labels is None and decoder_input_ids is None:
            decoder_input_ids = shift_tokens_right(
                query_input_ids, self.config.pad_token_id, self.config.decoder_start_token_id
            )
    ...
    def generate(
        ...
        **kwargs,
        ...
        encoder_outputs_tensor, encoder_attention_mask = self._get_combined_encoder_outputs(
            query_input_ids=query_input_ids,
            query_attention_mask=query_attention_mask,
            retr_input_ids=retr_input_ids,
            retr_attention_mask=retr_attention_mask,
        ...
        encoder_outputs = BaseModelOutput(last_hidden_state=encoder_outputs_tensor)
...
if __name__ == "__main__":
    ...
    train_dataloader = DataLoader(
        train_dataset, 
        batch_size=16, # Lower batch size might be needed due to larger memory footprint
        shuffle=True, 
    )
    ...
    model = CustomBart.from_pretrained('./models/bart_finetuned_knn')
    model.to(device)
    ...
    early_stopping=True   # Stop when a </s> is generated
)
